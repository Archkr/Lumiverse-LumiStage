...
export function Surface(props: {
  ...
  return <section class={`ls2-surface ${props.class ?? ""}`} data-tone={props.tone ?? "default"} data-padding={props.padding ?? "default"}>{props.children}</section>;
}

export function Field(props: { label: string; hint?: string; children: ComponentChildren; class?: string }) {
  return (
    <label class={`ls2-field ${props.class ?? ""}`}>
      <span class="ls2-field-label">{props.label}</span>
      {props.children}
      {props.hint && <span class="ls2-field-hint">{props.hint}</span>}
    </label>
  );
}
...
export function Toggle(props: { checked: boolean; onChange: (value: boolean) => void; label: string; hint?: string; disabled?: boolean }) {
  return (
    <label class="ls2-toggle-row" data-disabled={props.disabled}>
      <span class="ls2-toggle-copy"><strong>{props.label}</strong>{props.hint && <small>{props.hint}</small>}</span>
      ...
      <span class="ls2-toggle-track" aria-hidden="true"><span /></span>
    </label>
  );
}

export function Status({ tone = "neutral", children }: { tone?: "neutral" | "accent" | "success" | "warning" | "danger"; children: ComponentChildren }) {
  return <span class="ls2-status" data-tone={tone}><span class="ls2-status-dot" />{children}</span>;
}
...
export function ViewHeader(props: {
  ...
  return (
    <header class="ls2-view-header">
      <div class="ls2-view-heading">
        {props.eyebrow && <span class="ls2-eyebrow">{props.eyebrow}</span>}
        <h2>{props.title}</h2>
        <p>{props.description}</p>
      </div>
      {props.actions && <div class="ls2-view-actions">{props.actions}</div>}
    </header>
  );
}
...
export function SectionTitle(props: { title: string; description?: string; trailing?: ComponentChildren }) {
  return (
    <div class="ls2-section-title">
      <div><h3>{props.title}</h3>{props.description && <p>{props.description}</p>}</div>
      {props.trailing && <div class="ls2-section-trailing">{props.trailing}</div>}
    </div>
  );
}
...
export function EmptyState(props: { icon: IconName; title: string; description: string; action?: ComponentChildren }) {
  return (
    <div class="ls2-empty">
      <div class="ls2-empty-icon"><Icon name={props.icon} size={24} /></div>
      <strong>{props.title}</strong>
      <p>{props.description}</p>
      {props.action && <div class="ls2-empty-action">{props.action}</div>}
    </div>
  );
}

export function InlineNotice({ tone = "accent", children }: { tone?: "accent" | "success" | "warning" | "danger"; children: ComponentChildren }) {
  return <div class="ls2-notice" data-tone={tone} role="status"><Icon name={tone === "warning" || tone === "danger" ? "warning" : tone === "success" ? "success" : "info"} size={16} /><div>{children}</div></div>;
}
...
export function ProgressNotice({ client }: { client: LumiStageClient }) {
  ...
  return (
    <div class="ls2-global-notice" data-tone={notice?.tone ?? "info"} role="status">
      <div class="ls2-global-notice-copy">{notice?.message ?? progress?.message}</div>
      {progress && progress.total > 0 && <div class="ls2-progress"><span style={{ width: `${Math.min(100, progress.completed / progress.total * 100)}%` }} /></div>}
    </div>
  );
}
...
export function Segmented<T extends string>(props: { value: T; onChange: (value: T) => void; options: Array<{ value: T; label: string; icon?: IconName }>; label: string }) {
  return (
    <div class="ls2-segmented" role="radiogroup" aria-label={props.label}>
